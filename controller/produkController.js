...
const index = async (req, res) => {
    try {
        const result = await Produk.findAll({ exclude: ['password'] });
...
const show = async (req, res) => {
    try {
        ...
        const data = await Produk.findByPk(id_kategori, { include: Kategori, attributes: ['id_kategori'] });
...
const update = async (req, res) => {
    try {
        ...
        upload(req, res, async function (err) {
            ...
            const { nama_produk, id_kategori, harga, deskripsi} = req.body;
            const slug = slugify(nama_produk, { lower: true });

            // Update data produk di database
            await produk.update({ nama_produk, id_kategori, harga, deskripsi });
